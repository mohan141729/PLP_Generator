...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (currentView) {
      case 'landing':
        return <LandingPage onNavigate={navigateTo} isAuthenticated={isAuthenticated} />;
      case 'login':
        return (
          <AuthPage
            mode={authPageMode}
  ...
  // Footer should not be fixed; keep layout flexible
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex flex-col">
      ...
      <footer className={"shadow-inner bg-gray-900 text-gray-300 py-8 md:py-10"}>        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-md bg-gray-800 ring-1 ring-gray-700">
                ...
                <div>
                  <p className="text-base font-semibold text-gray-100">PLP Generator</p>
                  <p className="text-xs text-gray-400">Personalized Learning Path, made simple.</p>
                </div>
              </div>
              <p className="mt-4 text-sm text-gray-400">
                Create structured, AI-assisted learning paths and track your progress seamlessly.
              </p>
            </div>

            <div>
              <h4 className="text-xs font-semibold tracking-wider uppercase text-gray-400">Quick Links</h4>
              <ul className="mt-3 space-y-2 text-sm">
                <li><button onClick={() => navigateTo('dashboard')} className="hover:text-white">Dashboard</button></li>
                <li><button onClick={() => navigateTo('create')} className="hover:text-white">Create Path</button></li>
                <li><button onClick={() => navigateTo('metrics')} className="hover:text-white">Metrics</button></li>
                <li><button onClick={() => navigateTo('landing')} className="hover:text-white">Home</button></li>
              </ul>
            </div>

            <div>
              <h4 className="text-xs font-semibold tracking-wider uppercase text-gray-400">About</h4>
              <div className="mt-3 space-y-1 text-sm text-gray-400">
                <p>Built with React + Vite + Tailwind CSS</p>
                <p>Backend: Express + SQLite</p>
                <p>AI: Google Gemini</p>
              </div>
            </div>
          </div>

          <div className="border-t border-gray-800 mt-8 pt-4 text-xs text-gray-500 flex flex-col sm:flex-row items-center justify-between gap-2">
            <span>&copy; {new Date().getFullYear()} All rights reserved by <span className="text-gray-300 font-medium">Mohan D</span>.</span>
            <span className="hidden sm:inline">PLP Generator</span>
          </div>
        </div>
      </footer>
    </div>
